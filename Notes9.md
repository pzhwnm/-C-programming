...
int main()
{
    ...
    //ppfarr是一个数组指针，指针指向的数组有四个元素,指向的数组的每个元素的类型是一个函数指针，int(*)(int,int)
    return 0;
}
```
...
回调函数就是通过函数指针调用的函数。如果你把函数的指针或者地址作为参数传递给另一个函数，当这个指针被用来调用器所指向的函数时，我们就说这是回调函数。回调函数不是由该函数的实现方式直接调用，而是在特定的事件或条件发生时由另外的一方调用的，用于对该事件或者条件进行响应。
